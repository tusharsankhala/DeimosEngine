...
struct Transform
{
	...
	{
		m_rotation = math::inverse(math::Matrix4::lookAt(math::Point3(source.getXYZ()),
			math::Point3(target.getXYZ()), math::Vector3(0, 1, 0)));

		m_translation = m_rotation.getCol(3);
		m_rotation.setCol(3, math::Vector4(0, 0, 0, 1));
	}
...
struct tfCamera
{
	enum Lightype { CAMERA_PERSPECTIVE };
