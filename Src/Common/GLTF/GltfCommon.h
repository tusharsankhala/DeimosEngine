...
// Define a maximum no of shadows supported in a scene ( note, these are only for Spots and Directional ).
...
//
// Structures holding per frame constant buffer data
//
struct Light
{
	math::Matrix4	m_lightViewProj;
	math::Matrix4	m_lightView;
...
struct per_frame
{
	math::Matrix4	m_cameraCurrViewProj;
	math::Matrix4	m_cameraPrevViewProj;
	math::Matrix4	m_inverseCameraCurrViewProj;
...
public:
	...
	std::vector<Matrix2>				m_worldSpaceMats;			// world space matrices of each node after processing the heirarchy.
	std::map<int, std::vector<Matrix2>>	m_worldSpaceSkeletonMats;	// Skinning matrices, following the m_jointNodeIdx order.
...
private:
	...
	math::Matrix4 ComputeDirectionalLightOrthhographicmatrix(const math::Matrix4& mLightView);
