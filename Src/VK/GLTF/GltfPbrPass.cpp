...
namespace Engine_VK
{
		...
		AsyncPool* pAsyncPool)
	{
		...
		// Set bindings for the rneder target.
		//
		DefineList rtDefines;
		m_pRenderPass->GetCompilerDefines(rtDefines);

		// Load Brdf look up table from the PBR shader.
		//
		...
		//
		{
			SetDefaultMaterialParameters(&m_defaultMaterial.m_pbrMaterialParameters);

			std::map<std::string, VkImageView> texturebase;
			CreateDescriptorTableForMaterialTextures(&m_defaultMaterial, texturebase, pSkyDome, shadowMapViewPool, bUseSSAOMask);
		}
		...
		for (uint32_t i = 0; i < materials.size(); ++i)
		{
			...
			// Translate texture ID's into textureViews.
			//
		...
		if (j3.find("meshes") != j3.end())
		{
			...
			for (uint32_t i = 0; i < meshes.size(); ++i)
			{
				...
				for (uint32_t p = 0; p < primitives.size(); ++p)
				{
					...
					{
						// Set primitive's material, or set a default material if none was specified in the GLTF.
						//
						...
						// Make a list of all the attribute names our pass requires, in the case of a PBR pass we need them all.
						//
	...
	void GltfPbrPass::BuildBatchLists(std::vector<BatchList>* pSolid, std::vector<BatchList>* pTransparent, bool bWireframe)
	{

	}

	void GltfPbrPass::DrawBatchList(VkCommandBuffer commandBuffer, std::vector<BatchList>* pBatchList, bool bWireframe)
	{
	...
	{
		// Bind indices and vertices using the right offset into the buffer.
		//
		...
		//
		VkDescriptorSet descriptorSets[2] = { m_uniformsDescriptorSet, m_pMaterial->m_texturesDescriptorSet };
		uint32_t descriptorSetsCount =  (m_pMaterial->m_textureCount == 0) ? 1 : 2;
	...
	void GltfPbrPass::CreateDescriptors(int inverseMatrixBufferSize, DefineList* pAttributeDefines, PBRPrimitives* pPrimitive, bool bUseSSAOMask)
	{
		...
		m_pResourceViewHeaps->CreateDescriptorSetLayoutAndAllocDescriptor(&layout_bindings, &pPrimitive->m_uniformsDescriptorSetLayout, &pPrimitive->m_uniformsDescriptorSet);
		// Init descriptor sets for the constant buffers.
		//
		m_pDynamicBufferRing->SetDescriptorSet(0, sizeof(per_frame), pPrimitive->m_uniformsDescriptorSet);
		m_pDynamicBufferRing->SetDescriptorSet(0, sizeof(per_object), pPrimitive->m_uniformsDescriptorSet);
