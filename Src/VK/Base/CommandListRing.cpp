...
namespace Engine_VK
{
	...
	// 
	//---------------------------------------------------------------------------------------------------------
	void CommandListRing::OnCreate( Device* pDevice, uint32_t numberOfBackBuffers, uint32_t commandListPerBackBuffer, bool compute )
	{
		m_pDevice = pDevice;
		m_numberOfAllocators		= numberOfBackBuffers;
		m_commandListsPerBackBuffer = commandListPerBackBuffer;

		m_pCommandBuffers = new CommandBuffersPerFrame[ m_numberOfAllocators ]();

		// Create command allocators, for each frame in flight we want to have a single Command Allocator,
		// and <commandListsPerBackBuffer> command buffer.
		//
		for( uint32_t a = 0; a < m_numberOfAllocators; ++a )
		{
			...
			cmd.commandBufferCount	= commandListPerBackBuffer;
	...
	{
		// Release and delete the command allocators.
		for( uint32_t a = 0; a < m_numberOfAllocators; ++a )
		{
	...
	VkCommandBuffer CommandListRing::GetNewCommandList()
	{
		...
		// If hit increase commandListPerBackBuffer.
		assert ( m_pCurrentFrame->m_usedCls < m_commandListsPerBackBuffer );
